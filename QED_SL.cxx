...
const int nSample = 1;
const char *Sample[nSample]={"QED_SL"};
...
double norm_gUPC_1fsr_other =  (1.0089 * 0.8716 * 0.9252 * 0.8487 * 10000000*luminosity)/13450; // gUPC
//double W1 = 13450*luminosity/10000000;// gUPC
...
// double wt[nSample] = {norm_LbLSC};
  ...
  TFile *outf= new TFile(Form("unfolding_histograms_Bayes_QEDSL%s.root",mc_name.c_str()),"recreate");  
  ...
  //for (int i = 0; i < nSample; i++){
   ...
   //qed[i]->Add(Form("/Users/pranatijana/Documents/LightByLight/Workspace/plottingMacros/After_Dielectron/WithGen/QEDSC__forUnfolding_7thFeb.root"));
   //New
    ...
    //qed[i]->Add(Form("/Users/pranatijana/Documents/Unfolding/NewUnfold/RooUnfold/examples/RootFile/ForDielectron/mc_qed_sl_Costheatstar_GenReco_6thMarch_Test.root"));//Starlight
    qed[i]->Add(Form("/Users/pranatijana/Documents/Unfolding/NewUnfold/RooUnfold/examples/RootFile/ForDielectron/mc_qed_sl_Costheatstar_GenReco_8thMarch.root"));//Starlight
 ...
 // TH1D* hRecoDataInvmass_xSec         = new TH1D("hRecoDataInvmass_xSec","Invmass reco Data",50,0,100);
  ...
  TH1D* hUnfoDataCosthetastar_Invert_xSec             = new TH1D("hUnfoDataCosthetastar_Invert_xSec","Costhetastar unfo Invert Data",10,0,1);
   ...
   // for (Long64_t jentry=0; jentry<10;jentry++) {
    for (Long64_t jentry=0; jentry<nentries;jentry++) {
      ...
      // "===================      Fill gen Info here ======================     
     //cout << "Gen_sielept:" << qedR.gen_diEle_Pt << endl;
     if(abs(qedR.gen_Eta1) > 2.2 || abs(qedR.gen_Eta2) > 2.2 || qedR.gen_Pt1 < 2.0 || qedR.gen_Pt2 < 2.0 || qedR.gen_diEle_M < 5 || qedR.gen_diEle_Pt > 1) continue;
      ...
      hdiele_GenPt[i]->Fill(qedR.gen_diEle_Pt,W1);
  ...
  if(qedR.ok_trigger == 1) { //trigger 
   ...
   //For SL
    ...
    //For gUPC
    // double lumiNorm_gUPC = RecoID_SF_Electron(ET1) * RecoID_SF_Electron(ET2) * norm_gUPC_1fsr_other;  
    // std::vector<double> wt = {lumiNorm_gUPC};
...
//	responsePt.Fill(weighted_vSum_Pt, weighted_gen_diEle_Pt);
  ...
  responseCosthetastar.Fill(abs(qedR.costhetastar),abs(qedR.gen_diEle_Costhetastar),wt[i]);
	...
	responsePt.Miss(qedR.gen_diEle_Pt,W1);
...
/********************Matrix-plotting************/
        ...
        // mark7->DrawLatex(0.65, 0.97, "#scale[0.8]{1.647 nb^{-1} (5.02 TeV)}");
        c4->Update();

        c4->SaveAs("fig_AN/Pt_SL_ele.pdf");
        //
        ...
        for (int i = 0; i < responseMatrix2.GetNrows(); ++i) {
             for (int j = 0; j < responseMatrix2.GetNcols(); ++j) {
                  hResponseMatrix2->SetBinContent(i + 1, j + 1, responseMatrix2[i][j]/hdiele_GenRapidity[0]->GetBinContent(j+1));
                  }
        }
        ...
        c5->Update();
        c5->SaveAs("fig_AN/Rapidity_SL_ele.pdf");
        //
      ...
      mark8->SetTextFont(42);
       ...
       c6->Update();
       c6->SaveAs("fig_AN/InvMass_SL_ele.pdf");
       ...
       c7->Update();
       c7->SaveAs("fig_AN/Costhetastar_SL_ele.pdf");
...
//TFile *f1=new TFile("/Users/pranatijana/Documents/Unfolding/NewUnfold/RooUnfold/examples/RootFile/ForDielectron/Data_sumpt_7bin.root","r");
 TFile *f1=new TFile("/Users/pranatijana/Documents/LightByLight/Workspace/plottingMacros/Data_rap_11bin.root","r");
  ...
  TH1D *hdiele_Costhetastar_data   = (TH1D*)f1->Get("hCosthetastar_unfoldData");
      ...
      RooUnfoldBayes unfold_dataPt(&responsePt, hdiele_Pt_data, 3); // unfold data
  ...
  /**********************Covarinace matrix*******************************/
    TCanvas*c8 =  new TCanvas("c8","covarianceMatrixPt", 800,600);
       ...
       c8->Update();
       c8->SaveAs("fig_AN/Cov_Pt_ele_SL.pdf");
  ...
  TCanvas*c9 =  new TCanvas("c9","covarianceMatrixRapidity", 800,600);
       ...
       c9->Update();
       c9->SaveAs("fig_AN/Cov_Rapidity_ele_SL.pdf");
  ...
  TCanvas*c10 =  new TCanvas("c10","covarianceMatrixInvmass", 800,600);
       ...
       c10->Update();
       c10->SaveAs("fig_AN/Cov_Invmass_ele_SL.pdf");
   ...
   TCanvas*c11 =  new TCanvas("c11","covarianceMatrixCosthetastar", 800,600);
       ...
       c11->Update();
       c11->SaveAs("fig_AN/Cov_Costhetastar_ele_SL.pdf");
       /*****************************************************/
...
double chi2_pt, chi2_mass, chi2_rap, chi2_pt_smear, chi2_mass_smear, chi2_rap_smear;
 for(int j =0; j <= hGenPt_xSec->GetNbinsX()+1; j++ ) {
     if(hGenPt_xSec->GetBinContent(j) > 0){
      chi2_pt+= ((hGenPt_xSec->GetBinContent(j)-hUnfoDataPt_xSec->GetBinContent(j)) * (hGenPt_xSec->GetBinContent(j)-hUnfoDataPt_xSec->GetBinContent(j)))/(hGenPt_xSec->GetBinError(j)*hUnfoDataPt_xSec->GetBinError(j));
...
//smear
 for(int j =0; j <= hRecoMCPt_xSec->GetNbinsX()+1; j++ ) {
     if(hRecoMCPt_xSec->GetBinContent(j) > 0){
      chi2_pt_smear+= ((hRecoMCPt_xSec->GetBinContent(j)-hRecoDataPt_xSec->GetBinContent(j)) * (hRecoMCPt_xSec->GetBinContent(j)-hRecoDataPt_xSec->GetBinContent(j)))/(hRecoMCPt_xSec->GetBinError(j)*hRecoDataPt_xSec->GetBinError(j));
...
///
  ...
  hdiele_Rapidity[0]->SetLineColor(kRed);
  hdiele_Rapidity[0]->Draw("p");;
...
TH1D* subtractInvBkg(TH1D *hdata, TH1D *hqed, TH1D *hcep, const char* name) {
   ...
   for (int i = 1; i <= hdata->GetNbinsX(); i++) {
        ...
        double binError = hdata->GetBinError(i)/hdata->GetBinWidth(i);
...
TH1D* getXSecHist(TH1D *hist, const char* name, int nbins, float xmin, float xmax, float luminosity){
  ...
  TH1D *hans = new TH1D(name,"",nbins,xmin,xmax);
   ...
   //  double binCont  = hist->GetBinContent(i)/(luminosity);
     ...
     double binError = (binCont * sqrt(pow(hist->GetBinError(i)/hist->GetBinContent(i),2)));//before
  ...
  TH1D *hans = new TH1D(name,"",nRapbin, Rapbin);
   ...
   for (int i=1; i<=hist->GetNbinsX(); i++) {
     ...
     // double binError = hist->GetBinError(i);
      ...
      hans->SetBinContent(i, binCont);
      hans->SetBinError(i, binError/hist->GetBinWidth(i));
    }
